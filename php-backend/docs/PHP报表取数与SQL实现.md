...
- [生成日报流程与实现](./生成日报流程与实现.md) — 点击「生成日报」后的完整调用链
- [字段映射与取数实现](../../docs/字段映射与取数实现.md) — 通用取数原理（语言无关）
- [技术架构](../../docs/技术架构.md) — 全栈概览（以 Python 版为主）
- [README](../README.md) — PHP 版启动与迁移清单
...
对应代码：`src/Services/MeichongRules.php`
...
2. **取哪些行？** `date_filter_column`、`row_filters`、`exclude_sample`、`exclude_review`、`join_to_orders`、`benchmark_keys`
...
读取入口：`MappingRepo::forDataSource()` → `src/Services/MappingRepo.php`
...
查询入口：`CatalogResolver`、`SchemaService` → `src/Services/CatalogResolver.php`
...
**文件**：`src/Services/FactProvider.php`
...
**文件**：`src/Services/FieldAggregator.php`

在 `ReportEngine::aggregateFieldValues()` 中，对 `FactProvider` 拉回的数万行数据做：

| 步骤 | 方法 | 说明 |
|------|------|------|
| 构建日报上下文 | `buildDailyContext()` | 当日有效订单、样品单 ID、刷单 ID、当日退单、关联键集合 |
...
| 字段复用 | `resolvePartValue()` + `ref_field_code` | 引用已算出的其他逻辑字段；`benchmark_keys` 强制关联主表 |
...
刷单相关 5 个字段由 `ReviewImport::reviewFieldValues()` 从 `data_sources.config.review_orders` 汇总；物流费按单固定，不扫事实表列求和。

---

### 3.6 公式求值：`Formula`

**文件**：`src/Services/Formula.php`
...
**文件**：`src/Services/ReportEngine.php`
...
更细的逐步说明见 [生成日报流程与实现](./生成日报流程与实现.md)。
...
| 模块 | 路径 | 职责 |
|------|------|------|
| 出报引擎 | `src/Services/ReportEngine.php` | 组织取数、公式、落库 |
| 事实表加载 | `src/Services/FactProvider.php` | **唯一业务 SQL** |
| 内存聚合 | `src/Services/FieldAggregator.php` | 过滤、去重、聚合、组合 |
| 公式引擎 | `src/Services/Formula.php` | `{field:code}` 解析与求值 |
| 映射仓储 | `src/Services/MappingRepo.php` | 读 field_mappings + parts |
| Catalog | `src/Services/CatalogResolver.php` | 元数据查询 |
| 数据源配置 | `src/Services/DsSettings.php` | 日期主表、定时出报等 |
| 刷单导入 | `src/Services/ReviewImport.php` | review_orders → 字段值 |
| 美宠种子规则 | `src/Services/MeichongRules.php` | 模板行、默认映射 |
| 模板同步 | `src/Services/ReportLineSync.php` | template_lines → field_mappings |
| API 入口 | `src/Controllers/ApiController.php` | `/api/generate` 等 |
| 页面入口 | `src/Controllers/PagesController.php` | `/daily/generate` 等 |
